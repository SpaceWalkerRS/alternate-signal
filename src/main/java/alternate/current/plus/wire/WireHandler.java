...
//import alternate.current.plus.AlternateCurrentMod;
...
public class WireHandler {
	...
	public static class Directions {
		...
		public static int flowOpposite(int flow) {
			return ((flow >> 2) | (flow << 2)) & 0b1111;
		}
	...
	/**
	 * Iterate over all neighbor positions of the given wire. The iteration order is
	 ...
	 * shape updates.
	 */
	private void forEachNeighborPos(WireNode wire, Consumer<BlockPos> consumer) {
		...
		consumer.accept(front.relative(rightward));
		consumer.accept(back.relative(leftward));
		consumer.accept(front.relative(leftward));
		consumer.accept(back.relative(rightward));
		consumer.accept(front.relative(downward));
		consumer.accept(back.relative(upward));
		consumer.accept(front.relative(upward));
		consumer.accept(back.relative(downward));
		consumer.accept(right.relative(downward));
		consumer.accept(left.relative(upward));
		consumer.accept(right.relative(upward));
		consumer.accept(left.relative(downward));

		// far neighbors (6)
		consumer.accept(front.relative(forward));
		consumer.accept(back.relative(backward));
		consumer.accept(right.relative(rightward));
		consumer.accept(left.relative(leftward));
		consumer.accept(below.relative(downward));
		consumer.accept(above.relative(upward));
	}

	/**
	 * Iterate over all neighboring nodes of the given wire. The iteration order is
	 * the same as in {@link WireHandler#forEachNeighborPos}.
	 */
	...
	private void findWirePower(WireNode wire, boolean ignoreNetwork) {
		wire.connections.forEach(connection -> {
			...
			if (!ignoreNetwork || !neighbor.inNetwork) {
				...
				int flow = Directions.flowOpposite(connection.flow);

				wire.offerPower(power, flow);
			}
		});
	}
...
//			profiler.end();
	...
	private void buildNetwork() {
		for (int index = 0; index < network.size(); index++) {
			...
			forEachNeighborPos(wire, pos -> {
				WireConnection connection = wire.connections.get(pos);

				if (connection == null || !connection.behavior.a2b()) {
					return;
				}
	...
	private void transmitPower(WireNode wire) {
		forEachNeighborPos(wire, pos -> {
			...
			int flow = connection.flow;

			if (neighbor.offerPower(power, flow)) {
				queueWire(neighbor);
			}
		});
	}
