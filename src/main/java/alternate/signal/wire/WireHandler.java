...
//import alternate.signal.AlternateSignalMod;
...
public class WireHandler {
	...
	static final ConnectionSide[][] CONNECTION_UPDATE_ORDERS = {
		{
			...
			ConnectionSide.DOWN_WEST,
			ConnectionSide.UP_EAST,
			ConnectionSide.UP_WEST,
			ConnectionSide.DOWN_EAST,
			ConnectionSide.DOWN_NORTH,
			ConnectionSide.UP_SOUTH,
			ConnectionSide.UP_NORTH,
			ConnectionSide.DOWN_SOUTH
		},
		{
			ConnectionSide.NORTH,
			ConnectionSide.SOUTH,
	...
	private Node getNextNode(BlockPos pos, BlockState state) {
		return ((IBlockState)state).isWire() ? new WireNode(level, pos, state) : getNextNode().set(pos, state, true);
	}
	...
	private void findRoots(BlockPos pos) {
		...
		for (int iDir : FULL_UPDATE_ORDERS[wire.iFlowDir]) {
			...
			if (neighbor.isConductor(SignalTypes.ANY) || neighbor.isSignalSource(SignalTypes.ANY)) {
	...
	private int getExternalPower(WireNode wire) {
		...
		for (int iDir = 0; iDir < Directions.ALL.length; iDir++) {
			...
			if (neighbor.isConductor(signal)) {
				power = Math.max(power, getDirectSignalTo(wire, neighbor, Directions.iOpposite(iDir)));
			}
			if (neighbor.isSignalSource(signal)) {
				power = Math.max(power, neighbor.istate.getSignal(level, neighbor.pos, Directions.ALL[iDir], signal));
			}
	...
	private int getDirectSignalTo(WireNode wire, Node node, int except) {
		...
		for (int iDir : Directions.I_EXCEPT[except]) {
			...
			if (neighbor.isSignalSource(signal)) {
				power = Math.max(power, neighbor.istate.getDirectSignal(level, neighbor.pos, Directions.ALL[iDir], signal));
...
//			profiler.end();
	...
	private void updateShape(Node node, Direction dir, BlockPos neighborPos, BlockState neighborState) {
		...
		if (!state.isAir() && !((IBlockState)state).isWire()) {
